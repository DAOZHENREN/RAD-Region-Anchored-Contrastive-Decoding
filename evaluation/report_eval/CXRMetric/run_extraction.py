...
def calc_metric(gt_csv, pred_csv, out_csv, use_idf): # TODO: support single metrics at a time
    ...
    os.makedirs(gt_label_path, exist_ok=True)

    # run encode.py to make the semb column
    os.system(f"mkdir -p {cache_path}")
    os.system(f"python /app/MedHEval/code/evaluation/report_eval/CXRMetric/CheXbert/src/label.py -c {CHEXBERT_PATH} -d {cache_pred_csv} -o {pred_label_path}")
    os.system(f"python /app/MedHEval/code/evaluation/report_eval/CXRMetric/CheXbert/src/label.py -c {CHEXBERT_PATH} -d {cache_gt_csv} -o {gt_label_path}")
