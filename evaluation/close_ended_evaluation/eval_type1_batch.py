...
if __name__ == "__main__":
    ...
    slake_ori_path = f"/app/MedHEval/benchmark_data/Visual_Misinterpretation_Hallucination/close-ended/fine-grained/slake_qa_pairs.json"
    with open(slake_ori_path, 'r') as file:
        slake_ori = json.load(file)
        
    rad_ori_path = f"/app/MedHEval/benchmark_data/Visual_Misinterpretation_Hallucination/close-ended/fine-grained/rad_vqa_pairs.json"
    with open(rad_ori_path, 'r') as file:
        rad_ori = json.load(file)

    xray_ori_path = f"/app/MedHEval/benchmark_data/Visual_Misinterpretation_Hallucination/close-ended/fine-grained/xray_closed_pairs.json"
    ...
    mimic_ori_path = f"/app/MedHEval/benchmark_data/Visual_Misinterpretation_Hallucination/close-ended/fine-grained/mimic_cxr_closed_pairs.json"
    ...
    for baseline in baselines:
            slake_path = f"{root_path}/MedHEval/pred/llava_med_1.5/{baseline}/type1/close_ended/SLAKE/pred_{baseline}.jsonl"
            rad_path = f"{root_path}/MedHEval/pred/llava_med_1.5/{baseline}/type1/close_ended/VQA_RAD/pred_{baseline}.jsonl"
            xray_path = f"{root_path}/MedHEval/pred/llava_med_1.5/{baseline}/type1/close_ended/IU_Xray/pred_{baseline}.jsonl"
            mimic_path = f"{root_path}/MedHEval/pred/llava_med_1.5/{baseline}/type1/close_ended/mimic_cxr/pred_{baseline}.jsonl"
            res = eval_all(slake_path, rad_path, xray_path, mimic_path, slake_ori, slake_id_to_ori, rad_ori, rad_id_to_ori, xray_ori, xray_id_to_ori, mimic_ori, mimic_id_to_ori)
            res_path = f"{root_path}/MedHEval/pred/llava_med_1.5/{baseline}/type1/close_ended/{baseline}_res.txt"
            with open(res_path, 'w') as file:
                file.write(str(res))