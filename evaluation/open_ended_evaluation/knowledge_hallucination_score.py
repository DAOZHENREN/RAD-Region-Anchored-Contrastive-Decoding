...
# 用环境变量或直接写死都行（推荐环境变量，方便“一套代码接很多后端”）
OPENAI_BASE_URL = os.getenv("OPENAI_BASE_URL", "https://spark-api-open.xf-yun.com/v2")
OPENAI_API_KEY  = os.getenv("OPENAI_API_KEY", "mNmTFHXSeYPmoWCYFuHz:fpRodFxFpXVeGiIacOND")  # 很多本地后端不校验，就随便填
OPENAI_MODEL    = os.getenv("OPENAI_MODEL", "spark-x")  # 换成你后端实际的模型名
...
open_eval_prompt = ChatPromptTemplate.from_messages([
    ...
    HumanMessagePromptTemplate(
        prompt=PromptTemplate(
            template="The question, ground_truth and models' model_response: {gr_pairs}. \n\n {format_instructions}\n\n Ensure that you read the model answers carefully and evaluate its correctness. Your evaluation output in a json format, without extra explanation:",
...
open_eval_chain = open_eval_prompt | llm | eval_parser




def load_model_ans(model_name):
    ans_path = f"/app/MedHEval/code/baselines/Med-LVLMs/{model_name}/output/type2/open_ended/original/MIMIC-CXR_pairs.jsonl"
...
len(model_name_res_dict["llava-med-1.0"])
question_ids = list(model_name_res_dict["llava-med-1.0"].keys())
# question_ids
...
def process_single_id(args):
    ...
    try:
        gt = model_name_res_dict["llava-med-1.0"][_id]["ground_truth"]
        q = model_name_res_dict["llava-med-1.0"][_id]["question"]
        
        models_res = {
            # "CheXagent": model_name_res_dict["CheXagent"][_id]["model_answer"],
            "LLaVA_med": model_name_res_dict["llava-med-1.0"][_id]["model_answer"],
            # "LLaVA_med_15": model_name_res_dict["LLaVA-med-1.5"][_id]["model_answer"],
            # "LLaVA": model_name_res_dict["llava-v1.6"][_id]["model_answer"],
            # "LLaVA_13b": model_name_res_dict["llava-v1.6-13b"][_id]["model_answer"],
            # "LLM_CXR": model_name_res_dict["LLM-CXR"][_id]["model_answer"],
...
def process_with_threads(question_ids, model_name_res_dict, open_eval_chain, num_threads=3):
