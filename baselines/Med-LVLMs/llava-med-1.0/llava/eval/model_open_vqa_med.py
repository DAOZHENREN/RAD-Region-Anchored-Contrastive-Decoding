...
def patch_config(model_path):
    # model_path 可能是模型目录，也可能直接是 config.json 路径
    ...
    missing = [k for k in patch_dict.keys() if k not in cfg]
    if missing:
        print(f"{missing} not found in `{config_path}`, applying patch and saving.")
        cfg.update({k: patch_dict[k] for k in missing})
...
def eval_model(args):
    ...
    for i, line in enumerate(tqdm(questions)):
        # if line.get("question_type") == "type_2" or line.get("question_type") == "type4_Knowledge":
        #     pass
        # else:
        #     continue
