...
class GenerationMixin:
    ...
    def generate(
        ...
        #Med-OPERA's kwargs
        med_opera_decoding: Optional[bool] = None,
        anatomy_patch_dict: Optional[List[int]] = None,
        ...
        **kwargs,
        ...
        if generation_mode == GenerationMode.GREEDY_SEARCH  and not dola_decoding:
            ...
            return self.greedy_search(
                ...
                streamer=streamer,
                boost_factor=1.0,
                anatomy_patch_dict = anatomy_patch_dict,
                **model_kwargs,
            )
        elif dola_decoding:
            ...
            #OPERA 需要知道输入序列里哪些位置对应：image tokens response tokens（模型生成部分起点）
            ...
            #OPERA 里有 rollback/history window 逻辑，长度控制必须明确，不然它的状态管理会很麻烦。
        ...
        elif generation_mode == GenerationMode.BEAM_SEARCH and med_opera_decoding:
            assert generation_config.output_attentions, "OPERA decoding requires output_attentions=True!"   
            #OPERA 的核心就是看 self-attention 来做惩罚/回滚（rollback），所以必须拿到 attentions。
    ...
    def med_PAI_greedy_search(
        ...
        anatomy_patch_dict: Optional[dict] = None,
        **model_kwargs,
    ) -> Union[GenerateNonBeamOutput, torch.LongTensor]:
        r"""
        Generates sequences of token ids for models with a language modeling head using **greedy decoding** and can be
        ...
        while True:
            if synced_gpus:
                ...
                if this_peer_finished_flag.item() == 0.0:
                    break
            # 🌟[插入点 1: Med-OPERA 视觉先验注入 (完美修复升级版)] 🌟
            # =========================================================================
            target_patches = None
            boost_factor_to_apply = boost_factor  # 默认的强制定向偏置
            

            if anatomy_patch_dict is not None:
                tokenizer = getattr(self, "med_opera_tokenizer", None)
                if tokenizer is not None:
                    # 1. 🌟 安全解码 & 滑动窗口机制 🌟
                    # 过滤掉 <image> 等负数 token，防止 decode 崩溃
                    safe_ids = input_ids[0][input_ids[0] >= 0]
                    
                    # 只取最近生成的 30 个 Token！(实现“目光流转”，不被古老的上文干扰)
                    recent_ids = safe_ids[-5:] if len(safe_ids) > 5 else safe_ids
                    recent_text = tokenizer.decode(recent_ids, skip_special_tokens=True).lower()                    
                    # 2. 泛化解剖学词汇映射 (将普通器官词汇映射到 anatomy_patch_dict 中的有效 Key)
                    
                    anatomy_aliases = {
                        # === 心脏与纵隔区域 (Heart & Mediastinum) ===
                        "heart": ["cardiomegaly"], 
                        "cardiothoracic": ["cardiomegaly"],
                        "mediastinum": ["cardiomegaly", "mass"],  # 纵隔：通常包含心脏和大血管，Mass也常发于纵隔
                        "aorta": ["cardiomegaly"],                # 主动脉：借用中心脏血管区域的特征

                        # === 肺部主体 (Lungs - 泛指) ===
                        # 建议使用面积覆盖最广的几种病变取并集，代表整个肺野
                        "lung": ["infiltration", "pneumonia", "emphysema"],
                        "lungs": ["infiltration", "pneumonia", "emphysema"],
                        "pulmonary": ["infiltration", "pneumonia", "emphysema"],

                        # === 局部肺野与特征区域 (Specific Lung Regions) ===
                        "apex": ["mass", "pneumothorax"],         # 肺尖：常见肺结节/肿块和气胸的高发区
                        "apical": ["mass", "pneumothorax"],
                        "hilar": ["consolidation", "mass"],       # 肺门：常表现为实变或肿块
                        "hilum": ["consolidation", "mass"],
                        "base": ["effusion", "consolidation"],    # 肺底：常伴随积液或下叶实变

                        # === 胸膜与边缘 (Pleura & Edges) ===
                        # 结合你之前的热力图，Pleural Thickening 覆盖了很广泛的边缘
                        "pleura": ["effusion", "pleural thickening"], 
                        "pleural": ["effusion", "pleural thickening"],
                        "costophrenic": ["effusion"],             # 肋膈角：积液（Effusion）最先出现的标志性区域！极其精准。

                        # === 膈肌 (Diaphragm) ===
                        # Hernia 确实是绝佳的膈肌代表，加上积液的底部特征更稳
                        "diaphragm": ["hernia", "effusion"],
                        "diaphragmatic": ["hernia", "effusion"]
                    }
                    target_patches_set = set()
                    
                    # A. Level 1: 尝试精准匹配疾病名称 (修复了 enumerate 的 Bug)
                    for disease_name in anatomy_patch_dict.keys():
                        if disease_name in recent_text:
                            target_patches_set.update(anatomy_patch_dict[disease_name])
                            
                            
                            
                    # B. Level 2: 如果没找到具体疾病，尝试匹配解剖学名词
                    if target_patches is None:
                        for alias, mapped_diseases in anatomy_aliases.items():
                            if alias in recent_text:
                                # 确保映射的疾病名在你的字典中存在
                                for disease in mapped_diseases:
                                    if disease in anatomy_patch_dict:
                                        target_patches_set.update(anatomy_patch_dict[disease])
                                        
                                    
                    # C. Level 3: 终极 Fallback：如果是纯文本废话复读，触发“全局视觉唤醒”
                    if len(target_patches_set) > 0:
                        # 转回 List 传给底层
                        target_patches = list(target_patches_set)
                        
                        # print(f"[Med-OPERA] 开启多区域融合靶向干预! (共激活 {len(target_patches)} 个 Patch)")
                    else:
                        # D. Level 3: 终极 Fallback，全是废话，全局唤醒
                        # print("[Med-OPERA] Level 3: 模型陷入纯文本复读，触发全局视觉唤醒!")
                        target_patches = None
                        
            
            # =========================================================================
            base_model = getattr(self, "model", self)
            #  开启精准定向干预！(仅针对本次预测) 学术上叫“Zero-shot Targeted Intervention（零样本定向干预）”
            if target_patches is not None:
                # 适配 LLaVA 的架构，通常真正的层在 self.model.layers 下
                
                target_tensor = torch.tensor(target_patches, dtype=torch.long, device=input_ids.device)
                image_offset = 34
                for layer in base_model.layers:
                    # 注意：你要确保之前魔改的 LlamaAttention 里接收了这2个变量
                    layer.self_attn.med_opera_target_patches = target_tensor + image_offset 
                    layer.self_attn.med_opera_boost_factor = boost_factor_to_apply # 注意力偏置
            else:
                # 🚨 致命修复 2：如果当前步不需要看病灶，必须清空上一层的残留干预！
                # 否则模型会被永远锁死在上一个病灶上！
                for layer in base_model.layers:
                    layer.self_attn.med_opera_target_patches = None 
    ...
    def opera_beam_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            #         print(f"  attentions[{i}].shape = {attn.shape}")  # usually [B, H, Q, K]

            # if outputs.hidden_states is not None:
            #     print(f"num hidden_state tensors: {len(outputs.hidden_states)}")
            #     for i, hs in enumerate(outputs.hidden_states):
            #         print(f"  hidden_states[{i}].shape = {hs.shape}")  # usually [B, T, D]
            ...
            #max pooling（最大池化） 操作，具体是在 num_heads（注意力头）维度上，q*k个格子里各自取最大值
            ...
            for candidate_id in range(num_attn_candidates):
                ...
                input_ids_tmp = torch.cat([input_ids, candidate_tokens[:, candidate_id].unsqueeze(-1)], dim=-1) #[batch_size*num_beam,T+1]  多beam并行
            ...
            #返回最大 attention 分数及其对应的位置索引，每个候选拓展的最大注意力位置

            rollback_loc = rollback_locs.mode().values.data # [batch_size * num_beams] mode()求众数函数，得到每个beam的最持续关注位置
            rollback_loc = rollback_loc.mode().values.data # [1] 得到所有beam的共同最持续关注位置
            #这里并没有跨batch，因为batch_size等于1，跨了beam
            ...
            if history_rollback_locs is None:
                history_rollback_locs = [rollback_locs.mode().values.data[:, None]] #[:, None] 等价于 unsqueeze(1)，把形状从：[B]变成：[B, 1]
            else:
            ...
            #记录生成至今为止，每个beam每次生成时的最持续关注点（回溯点）

            candidate_token_scores -= penalty_weights * penalty_scores
            #candidate_token_scores是beam的logits topk对应的大小
            ...
            try:
                if all((rollback_loc_gather == rollback_loc).long().sum() > int(threshold) for _, rollback_loc_gather in enumerate(rollback_loc_gathers)):
                    #生成至今为止，每个beam每次生成时的最持续关注点和当前所有beam生成下一个token时的共同最持续关注点相同的个数大于阈值则触发回溯（长久持续关注=异常）
                    ...
                    beam_indices = history_states[-2]["beam_indices"]  #把每一步的 beam_idx 累积起来
                    ...
                    beam_idx = beam_idx.view(batch_size, num_beams) #beam_idx: 新 beam 来自哪条旧 beam
                    beam_next_tokens = beam_next_tokens.view(batch_size, num_beams) #beam_next_tokens: 当时给它接的 token
                    reject_token_pos = beam_idx * vocab_size + beam_next_tokens
                    if len(reject_token_pos_gather[rollback_pos]) > 0:
                        reject_token_pos = torch.cat([reject_token_pos_gather[rollback_pos], reject_token_pos], -1) #返回全部被拒绝过的token_pos(考虑了不同beam)
                    reject_token_pos_gather[rollback_pos] = reject_token_pos
                    next_token_logits = next_token_logits.scatter_(-1, reject_token_pos, -999.) #硬性屏蔽（hard mask）：直接设成 -999 几乎等价于“不允许选”
                    next_token_logits = next_token_logits.view(batch_size * num_beams, vocab_size)
                else:
                    assert False
            except:
    ...
    def med_opera_beam_search(
        ...
        threshold: Optional[int] = 15,
        boost_factor: Optional[float] = 0.5,
        anatomy_patch_dict: Optional[list] = None,
        num_attn_candidates: Optional[int] = 5, 
        window_size: Optional[int] = 512, 
        penalty_weights: Optional[float] = 1.0,
        ...
        **model_kwargs,
        ...
        reject_token_pos_gather = [[] for _ in range(window_size)]
        model_kwargs_ori = model_kwargs.copy()
        # print(model_kwargs["attention_mask"].shape, model_kwargs["inputs_embeds"].shape)
        original_prompt_len = input_ids.shape[1]
 
        target_patches = anatomy_patch_dict
        ...
        if target_patches is not None:
            # 适配 LLaVA 的架构，通常真正的层在 self.model.layers 下
            target_tensor = torch.tensor(target_patches, dtype=torch.long, device=input_ids.device)
            image_offset = key_position["image_start"]
            global_tensor = target_tensor + image_offset
            for layer in base_model.layers:
                
                layer.self_attn.med_opera_target_patches = global_tensor 
        ...
        if return_dict_in_generate:
            ...
            if self.config.is_encoder_decoder:
                ...
                return GenerateBeamDecoderOnlyOutput(
                    ...
                    hidden_states=decoder_hidden_states,
                    past_key_values=model_kwargs.get("past_key_values"),
                ) #这个函数把 beam search 的结果打包成 HuggingFace 的标准“生成输出对象”并返回
        else:
            return sequence_outputs["sequences"]
