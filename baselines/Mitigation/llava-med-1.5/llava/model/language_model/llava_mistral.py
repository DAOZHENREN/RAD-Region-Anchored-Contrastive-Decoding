...
class LlavaMistralForCausalLM(MistralForCausalLM, LlavaMetaForCausalLM):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            ) #把“文本 token 序列 + 图像”变成“语言模型可吃的 embedding 序列”
        ...
        if mask_idx is not None and past_key_values is None and masking_scheme is not None:
            ...
            for input_embed, idx in zip(inputs_embeds, mask_idx):
                ...
                if masking_scheme.lower() == "ones":
                    ...
                    # print(input_embed[idx+34], "after zeros")
                    # print("zeros")
        ...
        # # 1. 直接打印当前调用的 forward 函数所在的文件路径
        # forward_path = inspect.getsourcefile(super().forward)
        # print(f"========== 破案了！forward 函数藏在这个文件里: {forward_path} ==========")

        # # 2. 如果你想知道它是属于哪个父类的，还可以打印出来看看
        # print(f"========== 当前 self 继承的父类顺序是: {self.__class__.__mro__} ==========")
    ...
    def prepare_inputs_for_generation_m3id(
        ...
        **kwargs
        ...
        if inputs_embeds is not None and past_key_values is None:
            ...
            # model_inputs = {"inputs_embeds": inputs_embeds}
            inputs_embeds = torch.cat([inputs_embeds[:,:34,:], inputs_embeds[:,34+576:,:]], dim=1) #将图片的 embedding 从 inputs_embeds 中剔除掉
            # print(inputs_embeds.size(), "inputs_embeds")
            model_inputs = {"inputs_embeds": inputs_embeds}
        else:
