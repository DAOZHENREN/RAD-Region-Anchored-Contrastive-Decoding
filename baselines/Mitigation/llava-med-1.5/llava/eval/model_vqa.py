...
import sys
sys.path.append("/app/MedHEval/code/baselines/Mitigation/llava-med-1.5")
...
def eval_model(args):
    ...
    if args.baseline =="med_opera":
        model.med_opera_tokenizer = tokenizer 
        from Med_OPARA_files.med_opara_attention import apply_med_opera_patch
        apply_med_opera_patch(model,start_layer=3, end_layer=32)
        from Med_OPARA_files.medical_prior import CheXNetPriorProvider  
        prior_provider = CheXNetPriorProvider(weight_path='/app/CheXNet/model.pth.tar', device='cuda:0')
    ...
    if args.baseline == "med_PAI":
        from transformers.generation.logits_process import LogitsProcessorList
        from PAI_files.model_loader import init_cfg_processor
        model.med_opera_tokenizer = tokenizer 
        from Med_OPARA_files.med_opara_attention import apply_med_opera_patch
        apply_med_opera_patch(model,start_layer=10, end_layer=22)
        from Med_OPARA_files.medical_prior import CheXNetPriorProvider  
        prior_provider = CheXNetPriorProvider(weight_path='/app/CheXNet/model.pth.tar', device='cuda:0')
    ...
    for line in tqdm(questions):
        ...
        if "conversations" in line:
            ...
            if 'ground_truth_type' in line and line['ground_truth_type'] == 'binary':
                qs += " Please answer Yes or No."
        ...
        if args.baseline == "opera":
            key_position = {
                "image_start": 34, 
                "image_end": 34+576,
                "response_start": input_ids.shape[1]-1 + 576
            }
            # print("use opera")
            ...
            from Med_OPARA_files.medical_prior import CLASS_NAMES
            
            all_heat_map = prior_provider.get_all_14_heatmaps(image, llava_grid_size=24) 
            # print("input_image_tensor:", image_tensor.shape)
            # print("all_heat_map:", all_heat_map.shape)

            # from Med_OPARA_files.medical_prior import visualize_and_save_14_heatmaps
            # visualize_and_save_14_heatmaps(image, all_heat_map, save_path=f"/app/MedHEval/pred/images/demo_patient_{idx}.png")

            target_patch= prior_provider.get_all_topk_indices(all_heat_map, boost_p=0.3) # 14x5 5个boost点
            
            
            with torch.inference_mode():
                # print(image_tensor.unsqueeze(0).half().cuda())
                # print("1--------------")
                output_ids = model.generate(
                    input_ids,
                    images=image_tensor.unsqueeze(0).half().cuda(),
                    ...
                    max_new_tokens=512,
                    output_attentions=True,
                    med_opera_decoding=True,  #这里
                    boost_factor=0.5,
                    anatomy_patch_dict = target_patch,
                    key_position=key_position,
                    scale_factor=25,
                    threshold=15,
            ...
            with torch.inference_mode():
                output_ids = model.generate(
                    ...
                    stopping_criteria=[stopping_criteria])     
        elif args.baseline == "med_PAI":
            from Med_OPARA_files.medical_prior import CLASS_NAMES
            
            all_heat_map = prior_provider.get_all_14_heatmaps(image, llava_grid_size=24)
            anatomy_patch_map = prior_provider.get_all_topk_indices(all_heat_map, boost_num=10) # 14x5 5个boost点
            # print("anatomy_patch_map:", anatomy_patch_map)
            anatomy_patch_dict = {
                disease_name.lower(): indices 
                for disease_name, indices in zip(CLASS_NAMES, anatomy_patch_map)
            }
            logits_processor = (
                    init_cfg_processor(tokenizer=tokenizer, llm_model=model, questions=[cur_prompt], gamma=1.1, beam=1, start_layer=2, end_layer=32)
                ) #这里PAI
            ...
            image_tensor_cd = add_diffusion_noise(image_tensor, 500) #args.noise_step 这里添加
