    ...
    **model_kwargs,
    ...
    while True:
        ...
        if use_cd:
            ...
            cd_logits = logits_warper(input_ids, cd_logits) #VCD 算出来的 logits 并不是直接采样，而是仍然接入 HF 的标准采样控制链

            next_token_scores = cd_logits
            cd_probs = nn.functional.softmax(cd_logits, dim=-1)
            next_tokens = torch.multinomial(cd_probs, num_samples=1).squeeze(1)  #按概率随机采样 1 个 token（不是 argmax）
        else:
        ...
        if return_dict_in_generate:
            ...
            if output_attentions:
                ...
                if self.config.is_encoder_decoder:
                    cross_attentions += (outputs.cross_attentions,)  #encoder-decoder 模型：存 decoder attentions + cross attentions
