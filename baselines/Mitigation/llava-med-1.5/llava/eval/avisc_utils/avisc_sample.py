    ...
    while True:
        ...
        if use_avisc:
            ...
            if model_inputs_method.get("past_key_values") is None:
                ...
                #outputs.attentions 32层，每层[1,32,657,657]
                for logit in outputs.attentions:
                    
                    img_logit = logit.mean(dim=1)[:,-1, img_idx] #先将 32 个头的结果加起来除以 32，再取查询维度的最后一个 Token的图像注意力 [1,576]
                    
                    layer_img_att_portion.append(img_logit.sum())#576个注意力加起来
                    
                layer_img_att_portion = torch.stack(layer_img_att_portion, dim=0)#列表变成一维张量
                ...
                #被选择的layers的图像注意力（多头平均后）
                img_att_logits = att_logits.mean(dim=1) #torch.Size([1, 576])
                
                # except global context token masking
                mask_idx = torch.where(img_att_logits < img_att_logits.mean() + img_att_logits.std() * lamb)[1].unsqueeze(0) #非盲目图片token的下标
                # print(mask_idx)
            ...
            lu = torch.log_softmax(next_token_logits_m3id, dim=-1) #安全型log(softmax(x))
            m3id_logit = lc + ((1-gamma_t)/gamma_t)*(lc - lu) #(1-gamma_t)/gamma_t刚开始很小（0.02），随生成过程逐渐增大（t=10,0.22,t=100,6.38）,动态增大惩罚力度
            # print(f"gamma_t: {(1-gamma_t)/gamma_t}, max logit diff: {(lc - lu).max().item()}, m3id_logit max: {m3id_logit.max().item()}")
